...
const { isChildTask } = require('./sidebar-model.cjs');

const SCROLLBACK_CAP = 10000;

function layoutPaneStyles(layout) {
...
function tabsForWorkspace(tabs, workspaceId) {
  return (tabs || [])
    .filter((tab) => tab.workspace_id === workspaceId)
    .sort((a, b) => (a.number ?? 0) - (b.number ?? 0));
}

function focusedTabForWorkspace(tabs, workspaceId) {
...
function partitionTabs(tabs, workspaceId) {
...
function layoutForTab(layouts, tabId) {
